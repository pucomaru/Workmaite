...
from llm.llm_factory import StructuredOutputError, llm_factory
...
async def _extract_propose_node(state: ExtractionState) -> dict:
    ...
    response = await llm.ainvoke(
        [
            SystemMessage(
                content=extract_agendas_system(
                    org_dept_list=state.get("org_dept_list") or "정보 없음",
                    knowledge=state.get("knowledge"),
                )
            ),
    ...
    proposed = _parse_json_from_text(cast(str, response.content))
    if proposed is None:
...
async def _similar_agenda_hint(content: str, meeting_id: int | None) -> str:
    ...
    try:
        ...
        if similar:
            parts.append(
                "[유사 과거 아젠다 — 중복·이월 판단 참고]\n"
                + "\n".join(
                    f"- {s.get('title')} ({s.get('status') or '?'})"
                    for s in similar
                    if s.get("title")
                )
            )
        if dept_rows:
            parts.append(
                "[부서별 과거 담당 빈도 — 부서 추천 참고]\n"
                + ", ".join(
                    f"{r['dept']}({r['c']})" for r in dept_rows if r.get("dept")
                )
            )
...
async def extract_agendas_and_todos(
    ...
    response = await llm.ainvoke(
        [
            SystemMessage(
                content=extract_agendas_system(
                    org_dept_list or "정보 없음", knowledge=knowledge
                )
            ),
            HumanMessage(
                content=f"아래 문서에서 아젠다와 Todo를 추출해 주세요.{prev_hint}{graph_hint}\n\n{content[:8000]}"
            ),
        ]
    )
    parsed = _parse_json_from_text(cast(str, response.content))
    reason = re.sub(
        r"```(?:json)?\s*[\s\S]*?```", "", cast(str, response.content)
    ).strip()
...
async def extract_agendas_from_context(
    context_parts: List[str], org_dept_list: str, user_id: int = None
) -> dict:
    ...
    response = await llm.ainvoke(
        [
            ...
            HumanMessage(
                content=f"다음 컨텍스트를 바탕으로 과제를 추출해 주세요:\n\n{chr(10).join(context_parts)}"
            ),
        ]
    )
    return _parse_json_from_text(cast(str, response.content).strip()) or {"agendas": []}
...
async def chat_update_agendas(
    ...
    response = await llm.ainvoke(
        [
            SystemMessage(
                content=chat_extract_system(
                    meeting_context, org_dept_list, current_agendas_text
                )
            ),
            HumanMessage(content=message),
        ]
    )
    return _parse_json_from_text(cast(str, response.content).strip()) or {}
