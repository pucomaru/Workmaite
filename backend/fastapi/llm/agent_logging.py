...
def record_token_usage(
    ...
    """현재 log_agent_run 컨텍스트의 콜렉터에 사용량을 직접 누적한다 (P2).

    LangChain 콜백을 거치지 않는 호출(임베딩 등)의 토큰을 같은 AgentLog의
    token_usage_logs에 합산하기 위함. 활성 콜렉터가 없으면(데코 컨텍스트 밖) 무시한다.
    """
    collector = _token_collector_var.get()
    if collector is None:
        return False
    collector._add(model, prompt_tokens, completion_tokens)
    return True
