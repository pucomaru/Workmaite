...
★ 드리프트 방지 규칙 (중요):
  ...
  2. 같은 의미를 두 이름으로 만들지 않는다. (예: 아젠다↔세션은 `논의` 하나 — 과거 `다룸`/`진행`은 폐지)
     안건으로 들어오는 문서 관계는 도출(회의록·보고자료)뿐 — `참조`로 안건을 가리키지 않는다.
...
REL_MATRIX: dict[str, str] = {
    ...
    # ── 보고자료 ──
    "report→agenda": "도출",  # 보고자료에서 도출된 안건 (회의록과 동일 — 과거 ``에서 통일)
    "report→Meetings": "발제",
    "dept→report": "발제",
    # 안건으로 들어오는(agenda<-) 관계는 도출(회의록·보고자료)·담당(사람)·관련(안건)뿐.
