...
- 답변 끝에 근거를 반드시 알려주세요: 어떤 회의체·아젠다·보고서·회의록을 조회하였기에 결과가 그렇게 도출되었는지. 도구를 호출하지 않았으면 인용하지 마세요.
...
async def direct_agent_stream(
    ...
    *,
    ...
    hint_parts = []
    if meeting_id:
        hint_parts.append(f"(현재 보고 있는 회의체 meeting_id={meeting_id})")
    if upcoming_ctx:
        hint_parts.append(upcoming_ctx)
    hint = "\n".join(hint_parts) + "\n" if hint_parts else ""
    config = {
        "configurable": {
