    ...
    try:
        ...
        stt_data = [
            ...
            dict(session_id=s1.id, speaker_label="SPEAKER_00", speaker_user_id=U["김민준"].id,
                 content="오전 10시 됐네요. 참석자 확인되셨죠? 1차 AI 제품 전략 정기회의 시작하겠습니다. 오늘 안건은 Q1 실적 리뷰, AI개발팀 기술 성과, 경쟁사 분석, Q2 로드맵 확정 순서입니다.",
                 start_sec=0.0, end_sec=12.4, confidence=0.98),
            dict(session_id=s1.id, speaker_label="SPEAKER_03", speaker_user_id=U["관리자"].id,
                 content="네, 안녕하세요. 오늘 이서연 씨는 외부 미팅으로 불참이죠?",
                 start_sec=13.0, end_sec=18.5, confidence=0.97),
            dict(session_id=s1.id, speaker_label="SPEAKER_00", speaker_user_id=U["김민준"].id,
                 content="네, 맞습니다. 사전에 연락 받았고요, 오늘 내용은 공유드릴 예정입니다. 그럼 바로 Q1 실적부터 가겠습니다.",
                 start_sec=19.2, end_sec=28.0, confidence=0.97),
            # Q1 실적 리뷰
            dict(session_id=s1.id, speaker_label="SPEAKER_00", speaker_user_id=U["김민준"].id,
                 content="Q1 전체 매출 달성률은 목표 대비 94%입니다. 영업이익은 작년 동기 대비 12% 개선됐고, 고객 이탈률은 3.2% 감소했습니다. 수치 자체는 준수하지만 목표 미달 원인을 짚고 넘어가야 합니다.",
                 start_sec=29.5, end_sec=46.8, confidence=0.97),
            dict(session_id=s1.id, speaker_label="SPEAKER_03", speaker_user_id=U["관리자"].id,
                 content="94%면 아쉽긴 한데요. 영업이익 12% 개선은 상당한 수치네요. 매출 미달 원인이 뭔가요?",
                 start_sec=47.5, end_sec=57.2, confidence=0.96),
            dict(session_id=s1.id, speaker_label="SPEAKER_00", speaker_user_id=U["김민준"].id,
                 content="크게 두 가지입니다. 신규 엔터프라이즈 계약 두 건이 협상 장기화로 Q2로 넘어갔고요, B2C 신규 가입 전환율이 예상보다 약 5% 낮았습니다. 온보딩 개선 배포 시점이 3월 말이라 Q1에 반영이 안 됐습니다.",
                 start_sec=58.0, end_sec=76.3, confidence=0.96),
            dict(session_id=s1.id, speaker_label="SPEAKER_01", speaker_user_id=U["박준혁"].id,
                 content="전환율 이슈는 저희 팀이랑도 관련 있습니다. 온보딩 플로우 개선 작업이 3월 29일에 배포됐는데, 반영 기간이 이틀밖에 없었어요. Q2에는 효과가 나타날 거라 봅니다.",
                 start_sec=77.0, end_sec=92.5, confidence=0.95),
            dict(session_id=s1.id, speaker_label="SPEAKER_03", speaker_user_id=U["관리자"].id,
                 content="그럼 Q2 전환율 목표는 얼마로 잡고 있나요?",
                 start_sec=93.2, end_sec=98.8, confidence=0.98),
            dict(session_id=s1.id, speaker_label="SPEAKER_00", speaker_user_id=U["김민준"].id,
                 content="현재 대비 8% 이상 개선 목표입니다. 온보딩 개선 효과가 3~4%, 신규 콘텐츠 마케팅으로 나머지를 채울 계획입니다.",
                 start_sec=99.5, end_sec=111.0, confidence=0.97),
            # AI개발팀 기술 성과
            dict(session_id=s1.id, speaker_label="SPEAKER_01", speaker_user_id=U["박준혁"].id,
                 content="그럼 AI개발팀 Q1 기술 성과 공유드리겠습니다. 가장 큰 성과는 추론 속도 개선입니다. 평균 응답 시간을 340밀리초에서 210밀리초로 줄였습니다. 약 38% 개선이에요.",
                 start_sec=120.0, end_sec=136.5, confidence=0.97),
            dict(session_id=s1.id, speaker_label="SPEAKER_03", speaker_user_id=U["관리자"].id,
                 content="오, 38%면 상당한 개선이네요. 어떻게 가능했나요?",
                 start_sec=137.2, end_sec=143.0, confidence=0.98),
            dict(session_id=s1.id, speaker_label="SPEAKER_02", speaker_user_id=U["최지은"].id,
                 content="vLLM 기반 PagedAttention 기법을 적용했습니다. GPU 메모리를 페이지 단위로 관리해서 배치 처리 효율이 크게 올라갔어요. 특히 동시 요청이 20개 이상일 때 효과가 두드러집니다.",
                 start_sec=144.0, end_sec=160.8, confidence=0.94),
            dict(session_id=s1.id, speaker_label="SPEAKER_01", speaker_user_id=U["박준혁"].id,
                 content="추가로 Redis 기반 응답 캐싱도 구현 중입니다. 반복 쿼리 처리를 90% 이상 줄이는 게 목표고요, 이번 스프린트 안에 완성할 예정입니다.",
                 start_sec=161.5, end_sec=174.2, confidence=0.96),
            dict(session_id=s1.id, speaker_label="SPEAKER_00", speaker_user_id=U["김민준"].id,
                 content="사용자 체감으로는 어느 정도 차이가 나나요?",
                 start_sec=175.0, end_sec=180.3, confidence=0.98),
            dict(session_id=s1.id, speaker_label="SPEAKER_02", speaker_user_id=U["최지은"].id,
                 content="일반 사용자 기준으로 340밀리초는 살짝 느리다는 피드백이 있었는데, 200밀리초 이하면 거의 즉각적으로 느껴집니다. 베타 테스터 만족도가 17포인트 올랐어요.",
                 start_sec=181.0, end_sec=196.7, confidence=0.93),
            dict(session_id=s1.id, speaker_label="SPEAKER_03", speaker_user_id=U["관리자"].id,
                 content="경쟁사 대비로는 어느 수준인가요?",
                 start_sec=197.4, end_sec=202.0, confidence=0.97),
            dict(session_id=s1.id, speaker_label="SPEAKER_01", speaker_user_id=U["박준혁"].id,
                 content="현재 시장 평균이 280밀리초 정도입니다. 저희가 210밀리초면 상위 20% 수준이고요, 연말까지 150밀리초를 목표로 하고 있습니다.",
                 start_sec=203.0, end_sec=216.5, confidence=0.96),
            dict(session_id=s1.id, speaker_label="SPEAKER_00", speaker_user_id=U["김민준"].id,
                 content="좋습니다. 그 수치를 고객사 제안서에 적극 넣읍시다. 기술 스펙이 영업 무기가 될 수 있습니다.",
                 start_sec=217.2, end_sec=226.8, confidence=0.97),
            # 경쟁사 분석
            dict(session_id=s1.id, speaker_label="SPEAKER_00", speaker_user_id=U["김민준"].id,
                 content="다음 경쟁사 분석입니다. 박준혁 팀장님이 준비하신 내용 공유해주세요.",
                 start_sec=240.0, end_sec=248.5, confidence=0.98),
            dict(session_id=s1.id, speaker_label="SPEAKER_01", speaker_user_id=U["박준혁"].id,
                 content="네. 가장 큰 이슈는 OpenAI가 Assistants API를 대폭 업그레이드했다는 겁니다. 파일 검색과 코드 인터프리터가 무료 플랜에도 포함됐어요. 저희 유료 기능 일부와 겹칩니다.",
                 start_sec=249.2, end_sec=265.8, confidence=0.95),
            dict(session_id=s1.id, speaker_label="SPEAKER_02", speaker_user_id=U["최지은"].id,
                 content="Google Gemini 1.5 Pro도 컨텍스트 윈도우를 100만 토큰으로 늘렸습니다. 긴 문서 처리 측면에서 저희보다 우위입니다. 저희는 현재 32K입니다.",
                 start_sec=266.5, end_sec=281.0, confidence=0.93),
            dict(session_id=s1.id, speaker_label="SPEAKER_03", speaker_user_id=U["관리자"].id,
                 content="그러면 우리가 차별화할 수 있는 포인트가 뭔가요? 솔직히 말해주세요.",
                 start_sec=282.0, end_sec=290.4, confidence=0.97),
            dict(session_id=s1.id, speaker_label="SPEAKER_00", speaker_user_id=U["김민준"].id,
                 content="저는 두 가지를 봅니다. 하나는 기업 데이터 보안입니다. 저희는 온프레미스 배포가 가능하고, 고객 데이터가 외부로 나가지 않습니다. 다른 하나는 도메인 특화 성능입니다.",
                 start_sec=291.0, end_sec=308.7, confidence=0.97),
            dict(session_id=s1.id, speaker_label="SPEAKER_01", speaker_user_id=U["박준혁"].id,
                 content="맞습니다. 저희 RAG 파이프라인은 고객사 내부 데이터와 결합할 때 일반 모델 대비 정확도가 35% 높습니다. 지난달 엔터프라이즈 파일럿 결과로 입증됐습니다.",
                 start_sec=309.5, end_sec=325.2, confidence=0.96),
            dict(session_id=s1.id, speaker_label="SPEAKER_03", speaker_user_id=U["관리자"].id,
                 content="35% 개선이면 강력한 셀링 포인트네요. 이걸 케이스 스터디로 만들 수 있나요?",
                 start_sec=326.0, end_sec=334.5, confidence=0.97),
            dict(session_id=s1.id, speaker_label="SPEAKER_00", speaker_user_id=U["김민준"].id,
                 content="이미 준비 중입니다. 파일럿 참여 고객사 한 곳에서 사례 공개 동의를 받았습니다. 다음 달 안에 공식 케이스 스터디로 발행할 수 있을 것 같습니다.",
                 start_sec=335.2, end_sec=350.8, confidence=0.96),
            dict(session_id=s1.id, speaker_label="SPEAKER_03", speaker_user_id=U["관리자"].id,
                 content="좋아요. 마케팅팀 정하윤 팀장님과 협업해서 빠르게 진행해주세요.",
                 start_sec=351.5, end_sec=359.0, confidence=0.98),
            # Q2 로드맵 논의
            dict(session_id=s1.id, speaker_label="SPEAKER_00", speaker_user_id=U["김민준"].id,
                 content="그럼 Q2 핵심 목표 논의하겠습니다. 제가 세 가지를 제안드립니다. 첫째, 멀티모달 파일럿 런칭. 둘째, 엔터프라이즈 온보딩 단축. 셋째, RAG 성능 평가 자동화입니다.",
                 start_sec=375.0, end_sec=392.8, confidence=0.97),
            dict(session_id=s1.id, speaker_label="SPEAKER_01", speaker_user_id=U["박준혁"].id,
                 content="멀티모달은 인프라 투자가 선행돼야 합니다. 현재 GPU로는 이미지 처리를 추가하면 추론 속도가 다시 느려집니다. A100이 추가로 필요한데, 파일럿 규모라면 4장이 현실적입니다.",
                 start_sec=393.5, end_sec=412.0, confidence=0.95),
            dict(session_id=s1.id, speaker_label="SPEAKER_02", speaker_user_id=U["최지은"].id,
                 content="A100 4장이면 월 비용이 약 600만 원입니다. 8장이면 1,200만 원이고요. 파일럿 규모에서는 4장으로 충분히 검증 가능합니다.",
                 start_sec=413.0, end_sec=426.5, confidence=0.94),
            dict(session_id=s1.id, speaker_label="SPEAKER_03", speaker_user_id=U["관리자"].id,
                 content="Q3에 Series B 준비 중인데, 선투자로 어떻게 봐야 할까요? 600만 원이 부담은 아니지만 효과를 증명해야겠죠.",
                 start_sec=427.2, end_sec=440.8, confidence=0.96),
            dict(session_id=s1.id, speaker_label="SPEAKER_00", speaker_user_id=U["김민준"].id,
                 content="파일럿으로 진행하자는 게 그 이유입니다. 베타 고객사 5곳에 먼저 테스트하고, 반응 보고 Q3 확대 여부를 결정하면 됩니다. Series B 투자 유치 때 데모로도 활용할 수 있고요.",
                 start_sec=441.5, end_sec=459.3, confidence=0.97),
            dict(session_id=s1.id, speaker_label="SPEAKER_03", speaker_user_id=U["관리자"].id,
                 content="좋아요. 그렇게 하죠. 파일럿 대상 고객사는 어떻게 선정합니까?",
                 start_sec=460.0, end_sec=468.5, confidence=0.98),
            dict(session_id=s1.id, speaker_label="SPEAKER_00", speaker_user_id=U["김민준"].id,
                 content="현재 엔터프라이즈 고객 중 이미지 처리 수요가 있는 곳을 먼저 접촉합니다. 이서연 씨가 CRM 분석 중인데, 이번 주 안에 5개사 리스트 받겠습니다.",
                 start_sec=469.2, end_sec=483.7, confidence=0.96),
            dict(session_id=s1.id, speaker_label="SPEAKER_01", speaker_user_id=U["박준혁"].id,
                 content="인프라 발주는 제가 이번 주 금요일까지 진행하겠습니다. 4장 기준으로 리드타임이 2주 정도라, 5월 중순이면 파일럿 환경 구성 완료됩니다.",
                 start_sec=484.5, end_sec=498.8, confidence=0.96),
            dict(session_id=s1.id, speaker_label="SPEAKER_02", speaker_user_id=U["최지은"].id,
                 content="RAG 성능 평가 자동화는 저희가 이미 착수했습니다. RAGAS 프레임워크 기반으로 주간 성능 리포트 자동 생성 시스템을 다음 달 말까지 완성할 예정입니다.",
                 start_sec=499.5, end_sec=515.2, confidence=0.95),
            dict(session_id=s1.id, speaker_label="SPEAKER_00", speaker_user_id=U["김민준"].id,
                 content="엔터프라이즈 온보딩 단축은 어느 수준을 목표로 하면 될까요? 현재 평균 온보딩 기간이 며칠입니까?",
                 start_sec=516.0, end_sec=526.8, confidence=0.97),
            dict(session_id=s1.id, speaker_label="SPEAKER_01", speaker_user_id=U["박준혁"].id,
                 content="현재 평균 14일입니다. 계정 설정, 데이터 연동, 직원 교육 합쳐서요. 목표는 7일 이내입니다. 자동화 스크립트랑 가이드 영상 제작으로 절반 줄일 수 있다고 봅니다.",
                 start_sec=527.5, end_sec=544.3, confidence=0.95),
            dict(session_id=s1.id, speaker_label="SPEAKER_03", speaker_user_id=U["관리자"].id,
                 content="온보딩 7일이 되면 영업 사이클도 빨라지겠네요. 계약 후 바로 사용 가능하다는 게 강점이 될 수 있습니다.",
                 start_sec=545.0, end_sec=556.7, confidence=0.97),
            # 마무리 및 액션 아이템
            dict(session_id=s1.id, speaker_label="SPEAKER_00", speaker_user_id=U["김민준"].id,
                 content="좋습니다. 오늘 결정 사항 정리하겠습니다. 하나, GPU A100 4장 추가 발주, 박준혁 팀장 이번 주 금요일까지. 둘, 멀티모달 파일럿 대상 고객사 5곳 선정, 이서연 씨 이번 주 안. 셋, RAG 평가 자동화 다음 달 완성. 넷, 케이스 스터디 마케팅팀 협업 다음 달 발행. 맞죠?",
                 start_sec=570.0, end_sec=600.5, confidence=0.97),
            dict(session_id=s1.id, speaker_label="SPEAKER_01", speaker_user_id=U["박준혁"].id,
                 content="네, 맞습니다. GPU 발주 외에 파일럿 테스트 환경 세팅은 최지은 씨가 담당하면 될 것 같습니다.",
                 start_sec=601.2, end_sec=612.0, confidence=0.96),
            dict(session_id=s1.id, speaker_label="SPEAKER_02", speaker_user_id=U["최지은"].id,
                 content="네, 인프라 들어오면 바로 세팅 시작하겠습니다. 고객사 요구사항 미리 받아두면 더 빠르게 진행할 수 있을 것 같습니다.",
                 start_sec=613.0, end_sec=624.8, confidence=0.95),
            dict(session_id=s1.id, speaker_label="SPEAKER_03", speaker_user_id=U["관리자"].id,
                 content="Q2 매출 목표는 15% 성장인데, 오늘 논의한 로드맵으로 달성 가능하다고 보시나요?",
                 start_sec=625.5, end_sec=635.2, confidence=0.97),
            dict(session_id=s1.id, speaker_label="SPEAKER_00", speaker_user_id=U["김민준"].id,
                 content="엔터프라이즈 계약 두 건이 Q2로 넘어왔고, 전환율 개선으로 신규 유입도 늘어납니다. 멀티모달 파일럿이 후반기에 신규 계약으로 이어지면 15% 달성 가능하다고 봅니다. 보수적으로 봐도 11~12%는 나올 것 같습니다.",
                 start_sec=636.0, end_sec=656.5, confidence=0.97),
            dict(session_id=s1.id, speaker_label="SPEAKER_03", speaker_user_id=U["관리자"].id,
                 content="알겠습니다. 잘 정리해주셨어요. 다음 회의는 2주 뒤 같은 시간입니다. 오늘 결정 사항은 반드시 Workmaite에 문서화해주세요. 수고하셨습니다.",
                 start_sec=657.2, end_sec=671.8, confidence=0.98),
            dict(session_id=s1.id, speaker_label="SPEAKER_00", speaker_user_id=U["김민준"].id,
                 content="네, 감사합니다. 오늘 회의록은 오후 중으로 아카이브에 업로드하겠습니다. 액션 아이템 담당자분들 기한 꼭 지켜주세요. 다들 수고하셨습니다.",
                 start_sec=672.5, end_sec=685.0, confidence=0.97),

            # ── 스프린트 #8 리뷰 & 레트로 (약 10분) ──────────────────────
            dict(session_id=s9.id, speaker_label="SPEAKER_00", speaker_user_id=U["박준혁"].id,
                 content="스프린트 #8 리뷰 시작하겠습니다. 이번 스프린트 완료율이 71%입니다. 목표 대비 낮은데, 원인 공유하고 다음 스프린트 계획 잡겠습니다.",
                 start_sec=0.0, end_sec=12.5, confidence=0.97),
            dict(session_id=s9.id, speaker_label="SPEAKER_01", speaker_user_id=U["최지은"].id,
                 content="완료 못한 주요 원인은 외부 API 장애입니다. OpenAI API가 이번 스프린트에 세 번 다운됐고, 총 4시간 정도 영향을 받았어요. 폴백 로직이 없어서 작업이 전부 막혔습니다.",
                 start_sec=13.2, end_sec=28.7, confidence=0.95),
            dict(session_id=s9.id, speaker_label="SPEAKER_00", speaker_user_id=U["박준혁"].id,
                 content="그게 이번 레트로의 핵심 이슈입니다. 단일 API 의존성이 전체 파이프라인 장애로 이어지는 구조를 반드시 개선해야 합니다.",
                 start_sec=29.5, end_sec=42.0, confidence=0.96),
            dict(session_id=s9.id, speaker_label="SPEAKER_02", speaker_user_id=U["김민준"].id,
                 content="개선 방향은 어떻게 잡았나요?",
                 start_sec=43.0, end_sec=47.5, confidence=0.98),
            dict(session_id=s9.id, speaker_label="SPEAKER_00", speaker_user_id=U["박준혁"].id,
                 content="이번 스프린트에 멀티 프로바이더 폴백 구현을 최우선으로 넣었습니다. OpenAI 실패 시 자동으로 Anthropic Claude로 전환하는 방식입니다. 응답 포맷 정규화도 함께 작업합니다.",
                 start_sec=48.2, end_sec=63.8, confidence=0.97),
            dict(session_id=s9.id, speaker_label="SPEAKER_01", speaker_user_id=U["최지은"].id,
                 content="전환 지연은 얼마나 되나요? 사용자 입장에서 느껴지는 정도인지 궁금합니다.",
                 start_sec=64.5, end_sec=73.2, confidence=0.95),
            dict(session_id=s9.id, speaker_label="SPEAKER_00", speaker_user_id=U["박준혁"].id,
                 content="헬스체크 주기를 5초로 잡고, 장애 감지 후 전환까지 약 2초입니다. 총 7초 이내면 사용자는 잠깐 느리다는 느낌 정도만 받습니다.",
                 start_sec=74.0, end_sec=87.5, confidence=0.96),
            dict(session_id=s9.id, speaker_label="SPEAKER_02", speaker_user_id=U["김민준"].id,
                 content="Redis 캐싱 작업 진행 상황도 공유해주세요.",
                 start_sec=88.2, end_sec=93.7, confidence=0.98),
            dict(session_id=s9.id, speaker_label="SPEAKER_01", speaker_user_id=U["최지은"].id,
                 content="기본 구조는 완성됐고, 캐시 무효화 전략이 남아있습니다. TTL 기반으로 할지 이벤트 기반으로 할지 고민 중인데요, 사용자별 컨텍스트가 다르니 이벤트 기반이 맞을 것 같습니다.",
                 start_sec=94.5, end_sec=113.0, confidence=0.93),
            dict(session_id=s9.id, speaker_label="SPEAKER_00", speaker_user_id=U["박준혁"].id,
                 content="저도 이벤트 기반이 맞다고 봅니다. 데이터 변경 이벤트에 따라 관련 캐시만 선택적으로 무효화하는 게 효율적입니다. PR 올리면 제가 같이 리뷰하겠습니다.",
                 start_sec=114.0, end_sec=128.5, confidence=0.95),
            dict(session_id=s9.id, speaker_label="SPEAKER_01", speaker_user_id=U["최지은"].id,
                 content="다음 주 수요일까지 PR 올리겠습니다. 테스트 케이스도 같이 포함할게요.",
                 start_sec=129.2, end_sec=138.7, confidence=0.96),
            dict(session_id=s9.id, speaker_label="SPEAKER_02", speaker_user_id=U["김민준"].id,
                 content="이번 스프린트 다른 완료 항목들도 간단히 공유해주세요.",
                 start_sec=139.5, end_sec=146.0, confidence=0.97),
            dict(session_id=s9.id, speaker_label="SPEAKER_00", speaker_user_id=U["박준혁"].id,
                 content="vLLM PagedAttention 적용 완료, 추론 속도 38% 개선 확인됐습니다. 모니터링 대시보드 기본 구성 완료, Grafana 연동됐고요. CI 파이프라인 개선도 완료해서 빌드 시간이 40% 줄었습니다.",
                 start_sec=147.0, end_sec=165.8, confidence=0.96),
            dict(session_id=s9.id, speaker_label="SPEAKER_02", speaker_user_id=U["김민준"].id,
                 content="CI 빌드 시간 40% 단축은 개발 생산성에 많이 도움 되겠네요. 잘 됐습니다.",
                 start_sec=166.5, end_sec=175.2, confidence=0.97),
            dict(session_id=s9.id, speaker_label="SPEAKER_00", speaker_user_id=U["박준혁"].id,
                 content="다음 스프린트 주요 목표 공유하겠습니다. 첫째 멀티 프로바이더 폴백 완성, 둘째 Redis 캐싱 완성, 셋째 RAG 평가 파이프라인 초안 구성입니다. 이번 스프린트 교훈 살려서 완료율 90% 이상 목표입니다.",
                 start_sec=180.0, end_sec=198.5, confidence=0.97),
            dict(session_id=s9.id, speaker_label="SPEAKER_02", speaker_user_id=U["김민준"].id,
                 content="좋습니다. 외부 의존성 이슈 재발 방지를 위해 스프린트 중간에 리스크 체크 시간도 넣으면 어떨까요?",
                 start_sec=199.2, end_sec=210.8, confidence=0.97),
            dict(session_id=s9.id, speaker_label="SPEAKER_00", speaker_user_id=U["박준혁"].id,
                 content="좋은 의견입니다. 매주 수요일 10분짜리 리스크 체크 미팅 추가하겠습니다. 오늘 리뷰는 여기까지입니다. 다들 수고하셨습니다.",
                 start_sec=211.5, end_sec=225.0, confidence=0.97),
        ]
        for st in stt_data:
