...
def check_daily_token_budget(db: Session, user_id: int) -> None:
    ...
    used = (
        db.query(func.coalesce(
            func.sum(models.TokenUsageLog.prompt_tokens + models.TokenUsageLog.completion_tokens), 0))
        .join(models.AgentLog, models.TokenUsageLog.agent_log_id == models.AgentLog.id)
        .filter(models.AgentLog.user_id == user_id,
                models.TokenUsageLog.created_at >= since)
        .scalar()
    )
    if used >= DAILY_TOKEN_BUDGET:
        logger.warning(f"[Guard] user={user_id} 일일 토큰 예산 초과 ({used}/{DAILY_TOKEN_BUDGET})")
        raise HTTPException(
            status_code=429,
...
def idempotency_guard(key: str) -> None:
    ...
    if key in _recent_keys:
        raise HTTPException(status_code=409, detail="이미 처리된 요청입니다 (중복 차단).")
