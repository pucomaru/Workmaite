...
해당 자료의 적합성·완성도를 아래 6개 항목 기준으로 평가하고 JSON을 반드시 반환하세요.

[6개 평가 항목 — 총 100점]
1. 목적및배경   (max: 15점) — 보고 목적과 배경이 명확하게 제시되었는가
2. 현황분석     (max: 20점) — 현재 상황을 객관적 데이터/근거로 분석했는가
3. 핵심내용     (max: 20점) — 주요 내용이 논리적으로 구성되었는가
4. 실행계획     (max: 20점) — 구체적인 실행 방안과 일정이 있는가
5. 기대효과     (max: 15점) — 기대 효과와 성과 지표가 제시되었는가
6. 리스크및대안 (max: 10점) — 리스크와 대안이 검토되었는가

반환할 JSON 형식:
{
  "score": <각 항목 점수의 합계 정수>,
  "detail_scores": {
    "목적및배경":   {"score": <0-15 정수>, "max": 15, "comment": "<한 문장 평가>"},
    "현황분석":     {"score": <0-20 정수>, "max": 20, "comment": "<한 문장 평가>"},
    "핵심내용":     {"score": <0-20 정수>, "max": 20, "comment": "<한 문장 평가>"},
    "실행계획":     {"score": <0-20 정수>, "max": 20, "comment": "<한 문장 평가>"},
    "기대효과":     {"score": <0-15 정수>, "max": 15, "comment": "<한 문장 평가>"},
    "리스크및대안": {"score": <0-10 정수>, "max": 10, "comment": "<한 문장 평가>"}
  },
  "feedback": ["종합 피드백 항목1", "종합 피드백 항목2", ...],
  "matched_agendas": [
...
중요 채점 기준:
- score는 반드시 detail_scores의 각 항목 점수 합계와 일치해야 함
- 파일 내용이 없거나 "[파일 미첨부]" 상태이면 각 항목 최대 배점의 30% 이하로 채점하고 feedback에 "파일 내용 없음" 명시
- "[바이너리 파일]"이면 각 항목 최대 배점의 50% 이하로 채점
- matched_agendas: 후보 과제 목록에 있는 id만 사용, 없으면 빈 배열 [] 반환 (임의 id 지어내지 말 것)
- agendas: 다음 회의에서 다뤄야 할 아젠다 제안 (1-3개)
- related_depts: 유관부서 (그래프에 이미 존재하는 부서 우선, 2-4개)
- feedback: 각 항목 평가를 종합한 구체적 피드백 3-5개
