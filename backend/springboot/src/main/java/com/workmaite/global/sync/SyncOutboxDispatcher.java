...
/**
 * 아웃박스 디스패처 (P2-4).
 * - kick(): 커밋 직후 즉시 비동기 디스패치 (낮은 지연)
 * - poll(): 30초 주기로 잔여 pending 행 재처리 (실패 재시도 — DATA-1 유실 방지)
 * 단일 replica 전제(현 구성). 멀티 replica 확장 시 SKIP LOCKED 클레임으로 전환할 것.
 */
@Slf4j
@Component
public class SyncOutboxDispatcher {
    ...
    public SyncOutboxDispatcher(SyncOutboxRepository outboxRepository,
    ...
    private void dispatchPending() {
        ...
        try {
            List<SyncOutbox> batch = outboxRepository.findTop50ByStatusOrderByIdAsc(SyncOutbox.STATUS_PENDING);
            for (SyncOutbox row : batch) {
                try {
                    ...
                    row.markDone();
                    log.info("[SyncOutbox] {} {}:{} 원본 없음(404) — skip",
                            row.getOp(), row.getEntityType(), row.getEntityId());
                } catch (Exception e) {
                    row.markAttemptFailed(e.getMessage());
                    log.warn("[SyncOutbox] {} {}:{} 실패 (retry={}) — {}",
                            row.getOp(), row.getEntityType(), row.getEntityId(),
                            row.getRetryCount(), e.getMessage());
                }
    ...
    private void send(SyncOutbox row) throws Exception {
        ...
        String url = switch (row.getEntityType()) {
            case "meeting" -> delete
            ...
            case "member" -> {
                ...
                yield "/api/sync/member?meetingId=" + row.getEntityId()
                        + "&userId=" + userId + "&role=" + p.path("role").asText("member");
            }
            default -> throw new IllegalArgumentException("unknown entity_type: " + row.getEntityType());
        };

        HttpHeaders headers = new HttpHeaders();
        headers.set("X-Internal-Secret", internalSecret);
        restTemplate.exchange(aiUrl + url, delete ? HttpMethod.DELETE : HttpMethod.POST,
