...
const pwStrengthLabel = computed(
  () => ['', '매우 약함', '약함', '보통', '강함'][pwStrength.value] || '',
)
const pwStrengthColor = computed(
  () => ['', '#ef4444', '#f59e0b', '#3b82f6', '#10b981'][pwStrength.value] || '',
)
  ...
  <div class="popup-inner">
    ...
    <div v-if="step === 1">
      ...
      <div class="mb-3">
        ...
        <div v-if="form.password" class="mt-2">
          ...
          <div class="d-flex justify-content-between mt-1">
            ...
            <small :style="{ color: pwStrengthColor }" class="fw-semibold">{{
              pwStrengthLabel
            }}</small>
          </div>
          <div class="d-flex gap-3 mt-1 flex-wrap">
            <small :class="form.password.length >= 8 ? 'text-success' : 'text-muted'">
              <i
                :class="form.password.length >= 8 ? 'bi bi-check-circle-fill' : 'bi bi-circle'"
              ></i>
              8자 이상
            </small>
            <small :class="/[a-zA-Z]/.test(form.password) ? 'text-success' : 'text-muted'">
              <i
                :class="/[a-zA-Z]/.test(form.password) ? 'bi bi-check-circle-fill' : 'bi bi-circle'"
              ></i>
              영문 포함
            </small>
            <small :class="/\d/.test(form.password) ? 'text-success' : 'text-muted'">
              <i :class="/\d/.test(form.password) ? 'bi bi-check-circle-fill' : 'bi bi-circle'"></i>
              숫자 포함
            </small>
          </div>
        </div>
      ...
      <div class="mb-4">
        ...
        <div v-if="form.confirm && !pwMatch" class="invalid-feedback d-block small">
          비밀번호가 일치하지 않습니다
        </div>
