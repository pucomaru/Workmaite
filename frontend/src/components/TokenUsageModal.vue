      ...
      <div class="app-modal tum-modal">
        ...
        <div class="tum-date-bar">
          <div class="app-modal-field" style="flex:1;min-width:0">
            <label>시작일</label>
            <DateInput v-model="startDate" class="app-modal-input" :max="endDate" @keydown.enter="fetchUsage"/>
          </div>
          <span class="tum-sep">—</span>
          <div class="app-modal-field" style="flex:1;min-width:0">
            <label>종료일</label>
            <DateInput v-model="endDate" class="app-modal-input" :min="startDate" @keydown.enter="fetchUsage"/>
          </div>
          <button class="app-btn-primary" style="align-self:flex-end" :disabled="loading" @click="fetchUsage">
          ...
          <div class="app-modal-body">
            ...
            <div class="app-modal-field">
              ...
              <div v-if="!sec.ai_model.by_model.length" class="tum-m-row">
                <span style="font-size:12px;color:var(--text-muted,#94a3b8)">사용 내역 없음</span>
              </div>
              <template v-for="model in sec.ai_model.by_model" :key="model.model_name">
                <!-- 모델 행 -->
                <div class="tum-m-row tum-model-row">
                  <span class="tum-model-dot" :style="{background: modelColor(model.model_name)}"/>
                ...
                <!-- 컨텍스트 하위 행 -->
                <div v-for="ctx in model.by_context" :key="ctx.section" class="tum-m-row tum-ctx-row">
            ...
            <div class="app-modal-field">
              ...
              <div v-for="p in sec.stt.by_provider" :key="p.provider" class="tum-m-row">
                ...
                <span class="tum-m-note">
                  {{ p.minutes.toFixed(1) }}분 · {{ p.cost_per_min > 0 ? `$${p.cost_per_min}/분` : '무료' }}
                </span>
...
.tum-m-row {
  ...
  padding: 7px 0; border-bottom: 1px solid var(--border, #e2e8f0); font-size: 12px;
}
