...
async function sendFeedback(msg, rating) {
  ...
  let reason = null
  if (rating === -1)
    reason = (await promptDialog('어떤 점이 아쉬웠나요? (선택, 취소 시 사유 없이 전송)')) || null
  msg._fb = rating
  try {
...
function onComposerReady({ textareaEl }) {
  agentTextareaEl.value = textareaEl
}
    ...
    <div v-if="agentSidebarOpen" class="agent-right-sidebar" :style="{ width: sidebarW + 'px' }">
      ...
      >
        <div
          ...
          :key="i"
          class="agent-msg-row"
          :class="msg.role === 'planning' ? 'planning' : msg.role"
        >
          <!-- 사고 과정 블록 -->
            ...
            <div class="agent-planning-block" :class="{ done: msg.done, open: msg.open }">
              <button class="agent-planning-toggle" @click="msg.open = !msg.open">
                ...
                <span class="agent-planning-label">
                  <template v-if="!msg.steps.length">{{
                    msg.done ? '완료' : '분석 중...'
                  }}</template>
                  <template v-else>{{
                    msg.steps[msg.steps.length - 1].length > 58
                      ? msg.steps[msg.steps.length - 1].slice(0, 57) + '…'
              ...
              <div v-if="msg.open" class="agent-planning-steps">
                ...
                >
                  <span
                    v-if="step.includes('위임') || step.includes('라우팅')"
          ...
          <template v-else-if="msg.role === 'agent' && msg.content">
            ...
            <div
              class="agent-bubble agent theme-supervisor"
            ...
            <div
              v-if="i === 0 && (agentInfo.suggested?.length || agentInfo.suggestedAt?.length)"
              class="agent-suggested"
            >
              <!-- prettier-ignore -->
              <button
                v-for="s in agentInfo.suggested"
                :key="s"
                class="suggested-btn"
        ...
        <div
          v-if="
            agentLoading &&
            currentMessages[currentMessages.length - 1]?.role === 'agent' &&
