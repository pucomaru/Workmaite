  ...
  if (s.members?.length) {
    ...
    if (s.attendee_ids?.length) {
      try {
        ...
        members.value = (data.data ?? data).map(u => ({ userId: u.id, name: u.name, email: u.email, role: roleMap[u.id] || 'member' }))
      } catch {}
    }
  }
}, { immediate: true })
        ...
        <div class="app-modal-body">
          ...
          <div class="app-modal-field">
            ...
            <div class="datetime-split-input">
              ...
              <input type="text" v-model="form.timeOnly" class="datetime-split-time" placeholder="HH:MM" maxlength="5" />
            </div>
          </div>
          <div class="app-modal-field">
