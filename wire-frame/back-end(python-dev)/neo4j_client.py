...
async def get_meeting_graph_context(meeting_id: str | int | None) -> dict:
    ...
    SQLite 정수 ID (1, 2, 3)와 Neo4j 문자열 ID (mg-001, mg-002, ...) 모두 지원합니다."""
    if not meeting_id:
        return {}
    # ID 변환: 숫자면 mg-{N:03d} 형식으로, 아니면 그대로
    try:
        mid_neo4j = f"mg-{int(meeting_id):03d}"
    except (ValueError, TypeError):
        mid_neo4j = str(meeting_id)
    mid_fallback = str(meeting_id)

    try:
        # 두 ID 형식 모두 시도
        mg_rows = await run_cypher(
            """MATCH (mg:MeetingGroup) WHERE mg.id = $id1 OR mg.id = $id2
               RETURN mg.id AS neo_id, mg.title AS title, mg.purpose AS purpose, mg.status AS status LIMIT 1""",
            {"id1": mid_neo4j, "id2": mid_fallback},
        )
        if not mg_rows:
            return {}
        neo_id = mg_rows[0].get("neo_id", mid_neo4j)

        agenda_rows = await run_cypher(
            """MATCH (ag:Agenda)-[:OWNED_BY]->(mg:MeetingGroup {id:$id})
               OPTIONAL MATCH (p:Person)-[:ASSIGNED_TO]->(ag)
               RETURN ag.title AS title, ag.status AS status, p.name AS assignee LIMIT 20""",
            {"id": neo_id},
        )
        session_rows = await run_cypher(
            """MATCH (s:Session)-[:HELD_BY]->(mg:MeetingGroup {id:$id})
               RETURN s.title AS title, s.session_number AS num, toString(s.ended_at) AS ended_at
        ...
        return {
            "meeting": mg_rows[0] if mg_rows else {},
            "agendas": agenda_rows,
...
def graph_context_to_str(ctx: dict) -> str:
    ...
    if sessions:
        lines.append(f"[최근 세션]")
        for s in sessions:
            lines.append(f"  - {s.get('session_number','?')}회차: {s.get('title','')} ({s.get('ended_at','?')})")
